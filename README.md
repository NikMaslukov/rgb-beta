...
`BTCPayServer.Plugins.RGB.csproj` and hash-locked in `packages.lock.json`. No binary is tracked in
...
python3 scripts/verify_plugin_artifact.py BTCPayServer.Plugins.RGB.btcpay \
  --provenance strict \
  --package-cache "${NUGET_PACKAGES:-$HOME/.nuget/packages}"
```
