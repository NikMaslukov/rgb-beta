...
* publish under a different ID they own — which means changing `PackageId` in
  `packaging/RgbVerifyCffi.csproj`, the `PackageReference` in `rgb-beta.csproj`, and
...
```bash
# edit rgb-beta.csproj: <PackageReference Include="RgbVerifyCffi" Version="<VERSION>" />
...
```bash
dotnet publish rgb-beta.csproj -c Release -o <out>
