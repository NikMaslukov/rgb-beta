...
Rows marked **(opening)** are asserted with **`StartsWith`**, not `Contains`. As `Contains` they assert
nothing where a longer row subsumes them — in state 2 `exists but could not be loaded` already contains
`could not be loaded` — and the property they exist to pin is that the message *opens* with the
state-appropriate sentence. Their absence in the other states remains a `Contains` assertion.
...
| `exists but could not be loaded` | present-but-unloadable only (its *absence* elsewhere is implied by row 1's, which subsumes it — this row earns its place through T3(e)'s attribution assertion, not through an absence check) |
| `expected symbol` | missing-export only (`is the wrong version` is dropped here: row 2's opening contains it, so as a separate row it asserts nothing) |
| the exception type name | state 5 (probe threw) only (`self-check failed` is dropped here for the same reason — row 3's opening contains it) |
