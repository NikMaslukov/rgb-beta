...
— which registers the `DllImportResolver` (`:14`) — also moves from first-send to plugin load. Benign here, and **not** the hazard an earlier draft described: the constructor runs inside `DefaultProbe`, which runs inside `VerifyOrLog`'s guard — phase 1a's call site; `Verify` deliberately rethrows, so the same fault escapes there and phase 2 must account for it, so a throwing constructor is caught and reported as state 5 like any other probe fault — it does not escape as a bare `TypeInitializationException`.
...
- `internal static IEnumerable<string> RuntimeIdentifiers()` — currently **private** (`:42`, no access
  modifier), which is why a *reader* of T1 cannot check its hardcoded list against the source; widen to `internal` so a reader can see the RID list T1 hardcodes against — **not** so any test calls it; T1's rule is that it never does. The superseded justification was — T1 writes its expectations out literally, and this widening exists only so a reader can see the RID list the test hardcodes against; the superseded justification was
...
regression deleting the registration would leave the probe green — and, **measured, would not break real sends either**, since `RgbLib`'s native assets keep the `DllImport` binding. In *this* repo the mutant is a no-op, not a failure in either direction (**uncovered mutant 7**); where those assets are absent the resolver is load-bearing and its loss is fail-closed there. **That gap is not covered by any unit test, and cannot be in this repo.** Measured: with the registration deleted entirely, and separately with the resolver forced to `IntPtr.Zero`, the real `DllImport` still binds — `RgbLib`'s native assets already put the directory on the default search path. The existing binding tests compensate for nothing, and neither does T21. §3.1's live run is mandatory before merge because it is the only check that exercises the resolver under BTCPay's plugin ALC at all; it cannot **distinguish** the two, since none of its runs removes the registration.
...
| `unsupported` | must NOT appear in **any** state (§2 forbids inventing a supported-platform claim). **Asserted by T3, T4, T12, T14 and T20 explicitly — T12 and T14 included, since they bind the *emitted* log and writer text**.** — §3's mutual-exclusion catch-all covers only *state-specific* tokens, so without a named clause this row and the next can never fail |
...
was otherwise spread across six places. Every entry is host-equivalent, contrived, or (entry 7) a no-op in this repo. **No entry can produce a false-ACCEPT**: each either fails closed or has no observable effect here — that, not a blanket "everything fails closed", is the invariant this table establishes.
...
| 7 | delete `SetDllImportResolver`, or force `ResolveNative` to `IntPtr.Zero` | **neither a unit test nor §3.1 can prove the resolver is load-bearing in this repo** — `RgbLib`'s native assets already put `runtimes/<rid>/native/` on the default search path, so the real `DllImport` still binds | **no observable effect in this repo** — that is precisely why it is uncovered: `RgbLib`'s native assets keep the `DllImport` binding, so the mutant is a no-op here, not a failure in either direction. It is not *safe*, it is *invisible*: on any deployment where those assets are absent the resolver becomes load-bearing and its loss makes the native unloadable — fail-closed there. §3.1's runs deliberately do **not** attempt it: they exercise the honest build (native present, native removed, native corrupted) plus a live send, which together prove the **shipped** configuration binds — not that the resolver is necessary to it. An earlier revision of this cell claimed §3.1 supplied that evidence and that the mutant is fail-closed *here*; both claims are withdrawn |
...
3. **A live signet send**, because §1's refactor touches the live P/Invoke resolution path. Unit tests
   ...
   recorded here. **A gate rejection counts only under a stricter condition, because the obvious reading of it is vacuous:** this gate is fail-closed, so a send rejected *because the native never bound* looks exactly like a send rejected by the verifier, and taking any rejection as proof of binding would make this run — the only evidence that the **shipped configuration binds under BTCPay's plugin ALC** — prove nothing (it is not evidence that the resolver is *necessary*; see uncovered mutant 7). A rejection therefore passes only if the log carries a **verifier verdict that only the loaded native can produce** (a decode or validate result naming the contract, seal or amount) and carries **no** `RgbNativeUnavailableException`, `DllNotFoundException` or self-check failure. Otherwise the run is inconclusive and must be repeated. Either way the outcome is stated, not summarised as "worked".
