...
| `unsupported` | must NOT appear in **any** state (§2 forbids inventing a supported-platform claim). **Asserted by T3, T4, T12 and T20 explicitly** — §3's mutual-exclusion catch-all covers only *state-specific* tokens, so without a named clause this row and the next can never fail |
