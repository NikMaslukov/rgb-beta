...
- **No new test guards "a real `DllImport` still binds"** — the existing
  ...
  must not hijack rgb-lib), **T19** (it must actually delegate to the shared loop) and the live signet send. It is not extra *environmental* cover: like `RgbVerifyBindingTests.cs:67-72` it needs a staged
