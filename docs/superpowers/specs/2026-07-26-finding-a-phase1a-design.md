...
**Standing rule 2: an identifier assertion on a bare syntax tree binds nothing, so every Roslyn clause naming an identifier must also assert the absence of shadowing.** Measured: a `static bool DefaultProbe(...)` **local function** shadowing the class member compiles at 0 warnings under `TreatWarningsAsErrors`, satisfies every node assertion verbatim, and the real helper never runs — the same trick re-points T19's `ResolveBaseDir`/`TryLoadFromCandidates` clauses. Each such clause must therefore also assert that the parsed file declares **no** local function, local variable, parameter, field or nested type of that name. (Binding through a `CSharpCompilation` and comparing `ISymbol`s is the principled alternative; it needs the full reference set, and the anti-shadow assertion suffices here because these names exist in exactly one place by construction.)
