...
| `exists but could not be loaded` | present-but-unloadable only — a genuine absence assertion in every other state, **including state 1**, which shares this state's opening sentence and so implies nothing about this token |
