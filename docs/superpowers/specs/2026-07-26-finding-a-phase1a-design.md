...
**Revision:** 17 — split out of the phase-1 spec after its gate round 5, then four rounds of its own gate
...
makes the branch assertions non-vacuous.
...
| T2 | `SelfCheck_LoadsAndResolvesAllFourExports_DoesNotThrow` | injected fakes, probe+export reporting success ⇒ no throw, all four symbol names queried, **and neither sink receives anything**. Without the silence assertion an implementation that logs the actionable error and still returns success passes every unit test; only §3.1 run 1 would catch it | `RgbNativeSelfCheck` does not exist |
