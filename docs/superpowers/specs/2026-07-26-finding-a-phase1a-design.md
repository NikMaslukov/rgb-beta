...
| 5 | Bind through `CSharpCompilation` + `SemanticModel` (refs from `TRUSTED_PLATFORM_ASSEMBLIES`; assert no `CS0246`/`CS0012` and a non-null symbol **before** comparing); **every absence assertion scans the whole compilation** | a stub type in the enclosing namespace, or a `partial class` in an unparsed file |
