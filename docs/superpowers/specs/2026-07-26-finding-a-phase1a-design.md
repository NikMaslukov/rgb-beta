...
**Revision:** 16 — split out of the phase-1 spec after its gate round 5, then four rounds of its own gate
...
//              RgbVerifyNative.ResolveBaseDir(typeof(RgbVerifyNative).Assembly), out h, out s, out f);
//   static bool DefaultHasExport(IntPtr h, string name)
...
3. **Operator remediation — honest, and branching on what was actually observed.** The probe cannot
   ...
   - **The library loaded but an expected export is missing** → name the library that loaded and the
     ...
     the other two: the load succeeded, so `existedButFailed` is empty and nothing "failed to load". An
