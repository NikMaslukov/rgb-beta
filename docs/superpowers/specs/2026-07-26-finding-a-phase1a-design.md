...
| `build-native.sh` | every failure state |

Each state-specific token must be **absent** from the other two states — that mutual exclusion is what
