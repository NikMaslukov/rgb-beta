...
| 3 | The pinned identifier is never reassigned before use; a *pinned assignment* must be the only one to its target | `probe ??= AlwaysHealthy;` leaves the asserted node intact |
| 4 | No `#if`/`#elif`/`#else`, no `using` alias, no `using static` in any parsed file | `#if`-guarded code is disabled trivia, invisible to node assertions |
| 5 | Bind through `CSharpCompilation` + `SemanticModel` (refs from `TRUSTED_PLATFORM_ASSEMBLIES`, `Compile` items including `obj/*.GlobalUsings.g.cs`; assert no `CS0246`/`CS0012` and a non-null symbol **before** comparing); **every absence assertion scans the whole compilation**. Applies to names that must resolve to *this plugin's* members. For **BCL member references** — `Console.Error`, `NativeLibrary.Load` — assert the `MemberAccessExpression` syntactically instead, because requiring the BCL to bind has been measured to false-reject. This is sound only with two qualifications, both load-bearing. **(i)** Match on the *rightmost two* name components — `ma.Name` is `Load` and the qualifier's trailing identifier is `NativeLibrary` — never on the whole expression's text: a fully-qualified `System.Runtime.InteropServices.NativeLibrary.Load(p)` renders as a different string and would slip past an equality check, defeating the very absence assertion that keeps states 1–2 from collapsing into state 5. **(ii)** The rules-2-and-4 argument that no file declares or aliases `Console`/`NativeLibrary` must be read at rule 5's scope — **all plugin sources**, not the two or three files a clause names — since a declaration or alias in an unparsed file is precisely the hole rule 5 exists to close | a stub type in the enclosing namespace, or a `partial class` in an unparsed file |
