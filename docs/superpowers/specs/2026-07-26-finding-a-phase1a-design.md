...
Behavioural tests (T1–T4, T12, T14, T15, T18, T19, T20, T22, and T23 except (e), which is a regression guard per its row) are written and observed failing before the corresponding
...
**Mutation-tested (rev 25).** An independent reviewer built the §2 surface (0 errors, and no *new* warnings from the files this phase adds or edits), ran all specified tests (26/26 green, T17 non-vacuous against the real staged
...
| 3 | The pinned identifier is never reassigned **anywhere in the method**; a *pinned assignment* must be the only one to its target | `probe ??= AlwaysHealthy;` leaves the asserted node intact |
