...
**The two sinks must fail independently.** Acquiring the logger, formatting, and writing to each sink are
separately guarded: a throwing `ILoggerFactory`/`CreateLogger` must still leave the full message in the
