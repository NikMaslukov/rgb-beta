...
| T13 | 2 | `PluginStartup_UsesHardFailEntryPoint` | **Roslyn-parsed, and it MUST obey phase 1a's five standing rules for Roslyn clauses** (§5 of `2026-07-26-finding-a-phase1a-design.md`: assert over syntax *nodes* not node text; no shadowing declaration of a pinned name beyond the mandated per-file set; no reassignment of a pinned identifier; no conditional-compilation directives or `using` aliases/`using static` in the parsed file; and — rule 5, the one that subsumes the qualifier assertion — bind the invocation through a `CSharpCompilation` + `SemanticModel` and compare the `ISymbol` to `…Services.RgbNativeSelfCheck.Verify`, because a stub type declared in the enclosing namespace defeats every syntax-only form). Those rules were derived from measured defeats of the phase-1a call-site guard, and **T13 is the same guard for the hard-fail flip, so every one of them applies here — with more at stake, since this is the assertion that the plugin refuses to start without the native.** In particular the invocation must be **member-access-qualified with `RgbNativeSelfCheck`** (an unqualified `Verify(…)` is satisfied by a `private static void Verify(…)` on `RGBPlugin`, and by `using RgbNativeSelfCheck = …Stub;` — both measured against phase 1a's T15), and the body lookup must use `Body ?? (SyntaxNode)ExpressionBody`. Parse `RGBPlugin.cs` with `Microsoft.CodeAnalysis.CSharp`, locate `Execute`, and assert it contains an `InvocationExpression` naming the throwing entry point (`Verify`), **and — carried over from phase 1a's T15 — that the invocation statement's index is lower than the `LoadConfiguration` invocation's**; dropping that clause would let the call site drift after `LoadConfiguration` uncaught once the flip lands, **no** invocation naming `VerifyOrLog`, and that the invocation is a *live, unguarded statement* — it must be a **direct child statement of `Execute`'s body block**, with no `IfStatement`, `TryStatement`, loop, lambda, or `LocalFunctionStatement` in its ancestor chain and no unconditional `return` preceding it at that level. Weaker rules are provably vacuous: measured, `if (false) { Verify(); }`, `try { Verify(); } catch { }`, a `Verify()` inside an uncalled local function, and one placed after an unconditional `return` each satisfy "an invocation exists and VerifyOrLog does not", letting phase 2 be claimed hard-fail while behaviour stays log-only. A plain source-text match is likewise satisfied by a commented-out or `#if false` call. As the flip's only automated guard the rule must reject all four | phase 1's call site invokes `VerifyOrLog`, so it fails until the flip lands |
