...
| Source (in-tree, vendored) | [`nuget_packages/RgbLib.0.3.0-beta.18.nupkg`](../nuget_packages/RgbLib.0.3.0-beta.18.nupkg) |
...
| SHA-256 | `ffb05605b4ce90e1cbcc6a130a589537325d81837d4bfcc7b249a905a5eaeaf9` |
| NuGet `contentHash` (SHA-512, base64) | `775X/n9sh6yss77AEaKCW9jeL3vYzJyjVLnOllfNZE46LSKkNiDuN4ux2J5qlJEggXgK0M2OtCSDIkpL4BoYKg==` |

Verify the in-tree package matches the SHA-256 above:

```bash
sha256sum nuget_packages/RgbLib.0.3.0-beta.18.nupkg
```
...
- Every CI run uses `dotnet restore --locked-mode -p:ContinuousIntegrationBuild=true`.
  ...
  `nuget_packages/RgbLib.0.3.0-beta.18.nupkg`, or of any package on
  nuget.org, breaks the build instead of shipping to users.
...
- the SHA-256 of `BTCPayServer.Plugins.RgbUtexo.btcpay`,
...
| `publish the rgb-lib package to nuget.org with author signing` | **not yet** — see §5.5 |

### 5.5. Known follow-ups

- Move `RgbLib.0.3.0-beta.18.nupkg` out of the git tree and publish it
  from `UTEXO-Protocol/rgb-lib-c-sharp` to a public registry
  (GitHub Packages preferred; nuget.org with a code-signing certificate
  if classic author-signing is required by an external audit). The
  lockfile will continue to pin the same `contentHash` after the move.
