...
public class NetworkDefaultsTests
{
    ...
    public void Utexo_DefaultsToUtexoEndpoints()
    {
        ...
        Assert.Equal("https://esplora-api.utexo.com", utexo.ElectrumUrl);
        Assert.Equal("rpcs://rgb-proxy.utexo.com/json-rpc", utexo.ProxyEndpoint);
    }

    [Fact]
    public void BitcoinChainClientFactory_HttpsScheme_CreatesEsploraClient()
    {
        using var client = BitcoinChainClientFactory.Create("https://esplora-api.utexo.com");
        Assert.IsType<EsploraHttpClient>(client);
    }

    [Fact]
    public void BitcoinChainClientFactory_HttpScheme_RequiresAllowInsecure()
    {
        using var client = BitcoinChainClientFactory.Create("http://esplora-api.local:8080", allowInsecure: true);
        Assert.IsType<EsploraHttpClient>(client);

        Assert.Throws<InvalidOperationException>(() =>
            BitcoinChainClientFactory.Create("http://esplora-api.local:8080"));
    }

    [Fact]
    public void BitcoinChainClientFactory_SslScheme_CreatesElectrumClient()
    {
        using var client = BitcoinChainClientFactory.Create("ssl://electrum.iriswallet.com:50033");
        Assert.IsType<ElectrumClient>(client);
    }

    [Fact]
    public void BitcoinChainClientFactory_TcpScheme_RequiresAllowInsecure()
    {
        using var client = BitcoinChainClientFactory.Create("tcp://regtest.thunderstack.org:50001", allowInsecure: true);
        Assert.IsType<ElectrumClient>(client);
    }

    [Fact]
    public void BitcoinChainClientFactory_UnknownScheme_Throws()
    {
        Assert.Throws<InvalidOperationException>(() =>
            BitcoinChainClientFactory.Create("ftp://example.com"));
    }

    [Fact]
    public void BitcoinChainClientFactory_EmptyUrl_Throws()
    {
        Assert.Throws<InvalidOperationException>(() =>
            BitcoinChainClientFactory.Create(""));
    }
    ...
    [Fact]
    public void AllowsPlainElectrum_RegtestOnly_True()
    {
    ...
    [Fact]
    public void AllowsPlainElectrum_NonRegtest_False()
    {
        Assert.False(NetworkSettings.AllowsPlainElectrum("utexo"));
        Assert.False(NetworkSettings.AllowsPlainElectrum("Utexo"));
