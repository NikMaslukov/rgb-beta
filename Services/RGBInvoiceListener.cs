...
public class RGBInvoiceListener : IHostedService
{
    ...
    async Task RefreshAllWallets(CancellationToken ct)
    {
        ...
        {
            try { await _wallets.RefreshWalletAsync(w.Id); await ProcessSettledTransfers(w.Id, ct); }
            catch { }
        }
    }
    ...
    async Task ProcessSettledTransfers(string walletId, CancellationToken ct)
    {
        ...
        {
            try { assetIds = assetIds.Union((await _wallets.ListAssetsAsync(walletId)).Select(a => a.AssetId)).ToList(); }
            catch { }
        }
        ...
        {
            try { settled.AddRange((await _wallets.GetTransfersAsync(walletId, aid)).Where(t => t.Status == 2 && t.Kind is 1 or 2)); }
            catch { }
        }
